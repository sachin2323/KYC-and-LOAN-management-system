...
const CONFIG_FPATH = path.join(__dirname, "./ConnectionProfile.yml");
...
class FBClient extends FabricClient {
    ...
    return channel
      ...
      .then(function(results) {
        ...
        if (isProposalGood) {
          ...
          let transaction_id_string = requestData.txId.getTransactionID();
          let promises = [];
  ...
  query(requestData) {
    let channel = this.getChannel();
    return channel
      .queryByChaincode(requestData)
      .then(response_payloads => {
        // console.log(response_payloads.toString())
        // if (typeof response_payloads === "object") {
        //   return response_payloads
        // }
        let resultData = JSON.parse(response_payloads.toString("utf8"));
        return resultData;
      })
...
fabricClient.loadFromConfig(CONFIG_FPATH);
